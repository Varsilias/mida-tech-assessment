...
export const validateRequest =
  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      logger.error(
        `[MidaCoreException] - [ExceptionHandler] - [auth.middleware.validateRequest]: ${error.message}`,
      );
...
export const authCheck = async (req: IRequest, res: Response, next: NextFunction) => {
  try {
    ...
    logger.error(
      `[MidaCoreException] - [ExceptionHandler] - [auth.middleware.authCheck]: ${error.message}`,
    );
    ...
    if (error instanceof JsonWebTokenError) {
      return res.status(HttpStatus.BAD_REQUEST).json({
        status: false,
        message: "Invalid or malfunctioned jwt token", // this could be less explicit in order to mask the exact details to potential threat actors
